...
from dfl_torch.losses import LPIPSLoss, discriminator_gan_loss, generator_adversarial_loss, masked_reconstruction_loss
...
def _compute_losses(model, discriminator, warped, target, mask, gan_power, lpips_loss_fn=None, lpips_weight=1.0,
                     is_src=False):
    """Shared forward+loss computation for one src or dst batch. `is_src` gates the adversarial
    term (see module docstring — GAN loss only applies to the src reconstruction)."""
    ...
    return pred, pred_mask, loss, recon_loss, lpips_loss


@torch.no_grad()
def _evaluate(model, src_val_loader, dst_val_loader, device, lpips_loss_fn=None):
    """Mean masked-reconstruction (+ LPIPS, if enabled) loss over the held-out validation split,
    with the model in eval mode and no warp augmentation (see build_train_val_dataloaders)."""
    model.eval()
    total_loss, n = 0.0, 0
...
def train(
    ...
    for step in range(start_step, total_steps):
        ...
        with autocast_context(device_type):
            pred_src_src, _, src_loss, src_recon_loss, src_lpips = _compute_losses(
                model, discriminator, src_warped, src_target, src_mask, gan_power, lpips_loss_fn, lpips_weight, is_src=True,
            )
            _, _, dst_loss, dst_recon_loss, dst_lpips = _compute_losses(
                model, discriminator, dst_warped, dst_target, dst_mask, gan_power, lpips_loss_fn, lpips_weight, is_src=False,
            )
        ...
        if step % checkpoint_every == 0 and step > 0:
            val_loss = _evaluate(model, src_val_loader, dst_val_loader, device, lpips_loss_fn)
